...
# Pulizia dei dati
df['deaths'] = pd.to_numeric(df['deaths'], errors='coerce')  # Converti in numerico
df = df.dropna(subset=['deaths'])  # Rimuovi righe con valori mancanti

# Mappatura delle regioni per uniformità
region_mapping = {
    'Europe and Central Asia': 'Europa & Asia Centrale',
    'Latin America and Caribbean': 'America Latina & Caraibi',
    'Middle East and North Africa': 'Medio Oriente & Nord Africa',
    'East Asia and Pacific': 'Asia Orientale & Pacifico',
...
fig = px.histogram(df, 
                   ...
                   color="region",
                   nbins=20,
                   title="Distribuzione delle Morti per Diarrea per Regione (2019)",
                   labels={"deaths": "Tasso di Mortalità per Diarrea", "count": "Numero di Paesi"},
                   opacity=0.7,
                   template="plotly_white",
                   hover_data=['country'])
...
fig.update_layout(
    ...
    xaxis=dict(
        title="Numero di morti per diarrea (per 100,000 abitanti)",
        tickmode='linear',
        dtick=2
    ),
    yaxis=dict(
        title="Conteggio Paesi",
        dtick=5
    ),
    bargap=0.1
)
