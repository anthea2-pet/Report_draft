...
paesi = [
    "Malaysia", "Vietnam", "Peru", "Yemen", "Benin", "Chad", "Togo", "Comoros", "Bangladesh",
    "Nigeria", "Haiti", "Central African Republic", "Tunisia", "Zimbabwe", "Ghana", "Morocco",
    "Lesotho", "Madagascar", "Papua New Guinea"
]

# Filtra i dati
df_filtered = df[df["country"].isin(paesi)]

# Rimuove righe con valori nulli
df_filtered = df_filtered.dropna(subset=["Birth rate, crude (per 1,000 people)"])

# Calcola media del birth rate per anno
media_annuale = df_filtered.groupby("year")["Birth rate, crude (per 1,000 people)"].mean().reset_index()

# Crea il grafico
grafico = (
    ggplot(media_annuale, aes(x="year", y="Birth rate, crude (per 1,000 people)")) +
    geom_line(color="#D81B60", size=1.5) +  # rosa scuro
    labs(
        title="Average Birth Rate Over Time (Selected Countries)",
        x="Year",
